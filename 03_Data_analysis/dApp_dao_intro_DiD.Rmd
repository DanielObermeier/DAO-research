...
# how many treated dApps per group 
#df.panel %>% group_by(dapp_name) %>% summarise(treated = mean(intro_dao_start_prop)) 
 # %>% group_by(category) %>% summarise(obs = n(), treated = sum(treated)) %>% arrange(desc(obs))
...
df.panel.grouped_category %>% ggplot(aes(x = reorder(category, -treated), y = treated, lable = treated)) + geom_bar(stat = "identity", fill = "blue") + theme_tufte()
...
treated <- df.panel %>% group_by(dapp_name) %>% summarise(treated = mean(intro_dao_start_prop))
