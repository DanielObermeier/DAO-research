...
dao_dynamic_did_defi <- df.panel %>% filter(category == "DeFi")%>% filter(abs(days_rel_to_treatment) <= 300 | days_rel_to_treatment ==  Inf) %>% did2s(data = ., # %>% filter(abs(days_rel_to_treatment) <= 400) 
                                                                    yname = "transactions_log", 
                                                                    treatment = "intro_dao_start_prop",
                                                                    first_stage = ~ 0 | dapp_name + date_key,
